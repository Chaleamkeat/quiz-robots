...
Reigstering
   ...
   Check SignupPage
   Click Gender
    Input Text    id=first_name   Chaleaamkeat
    Input Text    id=last_name    Kumpung
    Input Text    id=password    123456789
    Select From List By Value    ${DAYS_DROPDOWN}    26
    Select From List By Value    ${MONTHS_DROPDOWN}    05
    Select From List By Value    ${YEARS_DROPDOWN}    2002
    Select Checkbox    id=newsletter
    Select Checkbox    id=optin
    Input Text    id=company    ABCD 
